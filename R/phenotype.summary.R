...
#' with the results. 
#' @param pth By default the working directory. Define the pth where you want the file to 
#' be saved
...
phenotype.summary <- function( input, variation = FALSE, nfactor = 10, showTable = TRUE, showFigures = FALSE, pth = getwd(), verbose = FALSE ){
    ...
    for( i in 1:nrow( ph )){
        ...
        if( length( unique( tt[,pcolumn])) <= nfactor){
            ...
            if( variation == FALSE){
                ...
                p <- p + ggplot2::ggtitle( colnames(selection)[1] )
    ...
    write.table( resultShowTable, file = paste0(pth, "/phenoSummary.txt"), 
                 col.names = TRUE, 
                 row.names = FALSE, 
                 quote     = FALSE, 
                 sep       = "\t" )
