...
```{r queryNhanesOutput, echo=FALSE}
library( knitr )
kable( head ( query ) , format = "rst")
```


```{r convertToGenophenoRobject}
genophenoExData <- queryGenoPheno( inputDataFile = paste0(system.file("extdata", 
                                                                      package="genophenoR"), 
                                                          "/queryOutput.txt"))
genophenoExData
```
...
```{r phenosumm, fig.width=10, fig.height=8, warning= FALSE}
